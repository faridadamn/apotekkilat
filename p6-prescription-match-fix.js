...
/* P6 discount, VAT, receipt, and journal fix.
   Bundled here because this file is already loaded by index.html. */
(function(){
  const ACCOUNTS=[
    {code:'2100',name:'PPN Keluaran',class:'Liabilitas'},
    {code:'4200',name:'Diskon Penjualan',class:'Beban'}
  ];
  const n=v=>Number(v)||0;
  const today=()=>new Date().toISOString().slice(0,10);
  const productById=id=>(DB.products||[]).find(p=>p.id===id);
  function ensureAccounts(){DB.chartOfAccounts=DB.chartOfAccounts||[];ACCOUNTS.forEach(a=>{if(!DB.chartOfAccounts.some(x=>x.code===a.code))DB.chartOfAccounts.push({...a});});}
  ...
  function computeTxTotals(tx){const items=tx.items||[];tx.grossSubtotal=items.reduce((a,it)=>a+n(it.originalPrice||it.normalPrice||it.price)*n(it.qty),0);tx.discountTotal=items.reduce((a,it)=>a+n(it.discountAmount),0);tx.subtotal=items.reduce((a,it)=>a+n(it.price)*n(it.qty),0);tx.tax=Math.round(tx.subtotal*.11);tx.vatTotal=tx.tax;tx.total=tx.subtotal+tx.tax;}
  function applyMeta(tx,ctx){if(!tx)return;let hasDiscount=false;tx.items=(tx.items||[]).map(it=>{const meta=(ctx||[]).find(x=>x.productId===it.productId&&(!it.unitCode||x.code===it.unitCode))||(ctx||[]).find(x=>x.productId===it.productId);if(!meta)return{...it,originalPrice:it.originalPrice||it.price,normalPrice:it.normalPrice||it.price,discountAmount:n(it.discountAmount)};const qty=n(it.qty)||meta.qty,discountAmount=Math.max(0,(meta.basePrice-meta.sellPrice)*qty);if(discountAmount>0)hasDiscount=true;return{...it,originalPrice:meta.basePrice,normalPrice:meta.basePrice,price:meta.sellPrice,discountAmount,discountPerUnit:Math.max(0,meta.basePrice-meta.sellPrice),priceListId:meta.best?.pl?.id||null,priceListName:meta.best?.pl?.name||null};});computeTxTotals(tx);tx.priceListIds=[...new Set((tx.items||[]).map(it=>it.priceListId).filter(Boolean))];tx.priceListNames=[...new Set((tx.items||[]).map(it=>it.priceListName).filter(Boolean))];return hasDiscount;}
  ...
  function rebuildSaleJournal(tx){if(!tx)return;ensureAccounts();computeTxTotals(tx);DB.journal=(DB.journal||[]).filter(j=>!(j.sourceType==='Sale'&&j.sourceId===tx.id));const gross=n(tx.grossSubtotal),disc=n(tx.discountTotal),net=n(tx.subtotal),vat=n(tx.vatTotal??tx.tax),total=n(tx.total),cost=hpp(tx.items),cash=(tx.payment==='Kredit'||tx.payment==='Piutang')?'1100':'1000';const entries=[debit(cash,total),...(disc?[debit('4200',disc)]:[]),credit('4000',gross),...(vat?[credit('2100',vat)]:[]),debit('5000',cost),credit('1200',cost)];const d=entries.reduce((a,e)=>a+e.debit,0),c=entries.reduce((a,e)=>a+e.credit,0);if(d===c)DB.journal.push({id:uid('j'),date:tx.time||Date.now(),sourceType:'Sale',sourceId:tx.id,note:`Penjualan ${tx.code||''}${disc?' · diskon '+fmt(disc):''}${vat?' · PPN '+fmt(vat):''}`,entries});}
  function rebuildAll(){ensureAccounts();(DB.transactions||[]).forEach(tx=>{computeTxTotals(tx);rebuildSaleJournal(tx);});if(typeof saveDB==='function')saveDB();}
  function wrapCheckout(){if(typeof checkout!=='function'||checkout.__discountVatWrapped)return;const old=checkout;checkout=window.checkout=function(){const before=new Set((DB.transactions||[]).map(tx=>tx.id)),ctx=cartCtx(),out=old.apply(this,arguments),tx=latestNew(before);if(tx){applyMeta(tx,ctx);rebuildSaleJournal(tx);if(typeof saveDB==='function')saveDB();}return out;};checkout.__discountVatWrapped=true;}
  function wrapModal(){if(typeof modal!=='function'||modal.__discountVatReceiptWrapped)return;const old=modal;modal=window.modal=function(title,html,onSave,opts){if(/Transaksi Berhasil/i.test(title||'')){const tx=[...(DB.transactions||[])].sort((a,b)=>(b.time||0)-(a.time||0))[0];if(tx){computeTxTotals(tx);const receipt=`ApotekKilat\n${'-'.repeat(28)}\n${tx.code||''}\n${new Date(tx.time).toLocaleString('id-ID')}\n${'-'.repeat(28)}\n`+(tx.items||[]).map(it=>`${it.name} x${it.qty}\n  Normal: ${fmt(n(it.originalPrice||it.normalPrice||it.price)*n(it.qty))}${n(it.discountAmount)?`\n  Diskon: -${fmt(it.discountAmount)} (${it.priceListName||'Price List'})`:''}\n  Net: ${fmt(n(it.price)*n(it.qty))}`).join('\n')+`\n${'-'.repeat(28)}\nSubtotal Normal: ${fmt(n(tx.grossSubtotal))}\nDiskon: -${fmt(n(tx.discountTotal))}\nSubtotal Kena PPN: ${fmt(n(tx.subtotal))}\nPPN 11%: ${fmt(n(tx.vatTotal??tx.tax))}\nTOTAL: ${fmt(n(tx.total))}\nBayar: ${tx.payment||'-'}`;html=`<div class="receipt">${esc(receipt)}</div>`;}}return old.call(this,title,html,onSave,opts);};modal.__discountVatReceiptWrapped=true;}
  function patchRender(){if(typeof render!=='function'||render.__discountVatRenderWrapped)return;const old=render;render=window.render=function(){ensureAccounts();const out=old.apply(this,arguments);ensureAccounts();return out;};render.__discountVatRenderWrapped=true;}
  ensureAccounts();wrapModal();wrapCheckout();rebuildAll();patchRender();
  window.ApotekKilatDiscountJournalFix={cartCtx,applyMeta,rebuildAll,rebuildSaleJournal,ensureAccounts};
})();
